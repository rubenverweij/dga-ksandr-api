source("api-set_column_types.R")

xgboost_data <- function(full_data, Y){
  
  try(full_data$Datum <- lubridate::as_date(full_data$Datum,format = '%d-%m-%Y'))
  # If duval is the variable of interest, define it based on the triangle indications
  if (Y == 'duval'){
    full_data$duval <- NA
    # Remember, 1 is no indication, 0 is an indication
    for(i in 1:NROW(full_data)){
      if((full_data$T1[i] == 0|| full_data$T4[i]==0 || full_data$T5[i]==0|| full_data$P1[i] == 0|| full_data$P2[i] == 0)){
        full_data$duval[i] <- 1
      }else{
        full_data$duval[i] <- 0
      }
    }
    variablen <- c("UN","Datum","H2","C2H6",
                   ...
                   "P9599","P99","Markerkleur","TDCG","duval")
  }
  else{
    # Variabelen =: X + Y
    variablen <- c("UN","Datum","H2","C2H6",
  ...
  result <- list(mData = mData_xgboost, UN = full_data$UN, Datum = full_data$Datum, sSplit = split, mTest_data = feats)
  return(result)
}